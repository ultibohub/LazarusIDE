...
type
  ...
  TGtk3StatusBar = class(TGtk3Panel)
  strict private
    class function StatusBarPanelDraw(AWidget: PGtkWidget; ACairo: PCairo_t;
      AData: GPointer): GBoolean; cdecl; static;
    class procedure StatusBarLayoutSizeAllocate(AWidget: PGtkWidget;
      AGdkRect: PGdkRectangle; AData: gpointer); cdecl; static;
    class function SizeGripDraw(AWidget: PGtkWidget; ACairo: PCairo_t;
      AData: gpointer): GBoolean; cdecl; static;
    class function SizeGripButtonPress(AWidget: PGtkWidget;
      AEvent: PGdkEventButton; AData: gpointer): GBoolean; cdecl; static;
    class procedure SizeGripRealize(AWidget: PGtkWidget;
      AData: gpointer); cdecl; static;
  private
    FBox: PGtkWidget;
    FSizeGrip: PGtkWidget; //GtkDrawingArea at right end, mimics SizeGrip, nil when SizeGrip=False
    procedure ConnectPanelEvents(ALabel: PGtkLabel; APanelIndex: Integer);
    procedure FixLastPanelExpand;
  protected
  ...
  public
    ...
    property MDIArea: TGtk3MDIArea read FMDIArea;
    function MDIWorkspace: TGtk3MDIWorkspace;
    procedure MDIChildActivated(AFrame: TGtk3MDIChildFrame);
    procedure MDIChildDeactivated(AFrame: TGtk3MDIChildFrame);
    function MDIChildCloseQuery(AFrame: TGtk3MDIChildFrame): Boolean;
    procedure MDIChildStateChanged(AFrame: TGtk3MDIChildFrame);
    procedure MDINoActiveChild(Sender: TObject);
  end;
  ...
  TGtk3MDIChildWindow = class(TGtk3Window)
  private
    FMDIFrame: TGtk3MDIChildFrame;
    procedure ApplyMDIPosition(AForm: TCustomForm);
  protected
    function CreateWidget(const {%H-}Params: TCreateParams): PGtkWidget; override;
    procedure setText(const AValue: String); override;
  public
    ...
    procedure SetBounds(ALeft, ATop, AWidth, AHeight: Integer); override;
    procedure Activate; override;
    property MDIFrame: TGtk3MDIChildFrame read FMDIFrame;
  end;

  { TGtk3MDIArea }

  TGtk3MDIArea = class(TGtk3Widget)
  private
    FWorkspace: TGtk3MDIWorkspace;
    FAppliedX, FAppliedY: Integer;
    FAppliedW, FAppliedH: Integer;
  public
    constructor Create(const AMDIForm: TWinControl); reintroduce;
    destructor Destroy; override;
    function getClientRect: TRect; override;
...
begin
  ...
  g_signal_connect_data(FWidget, 'leave-notify-event', TGCallback(@MouseLeaveNotify), Self, nil, G_CONNECT_DEFAULT);
...
const
  //shade(@theme_bg_color, factor) — GTK3 re-evaluates this on every theme change,
  //so bevel colors always match the active theme. factor<1=darker, factor>1=lighter.
  Gtk3StatusBarBevelCSS =
    'label.statusbar-bevel-lowered {' +
    '  border-width: 1px; border-style: solid;' +
    '  border-top-color: shade(@theme_bg_color, 0.65);' +
    '  border-left-color: shade(@theme_bg_color, 0.65);' +
    '  border-bottom-color: shade(@theme_bg_color, 1.4);' +
    '  border-right-color: shade(@theme_bg_color, 1.4); }' +
    'label.statusbar-bevel-raised {' +
    '  border-width: 1px; border-style: solid;' +
    '  border-top-color: shade(@theme_bg_color, 1.4);' +
    '  border-left-color: shade(@theme_bg_color, 1.4);' +
    '  border-bottom-color: shade(@theme_bg_color, 0.65);' +
    '  border-right-color: shade(@theme_bg_color, 0.65); }';

var
  Gtk3StatusBarBevelProviderInstalled: Boolean = False;
...
begin
  ...
  case APanel.Bevel of
    pbLowered: gtk_style_context_add_class(
      gtk_widget_get_style_context(Result.FLabel), 'statusbar-bevel-lowered');
    pbRaised: gtk_style_context_add_class(
      gtk_widget_get_style_context(Result.FLabel), 'statusbar-bevel-raised');
  end;
  ...
  case APanel.Alignment of
    taCenter: Result.FLabel^.set_xalign(0.5);
    taRightJustify: Result.FLabel^.set_xalign(1.0);
  else
  ...
  TGtk3StatusBar(FStatusBar).ConnectPanelEvents(Result.FLabel, Self.Count - 1);
  gtk_box_pack_start(ABox, Result.FLabel, False, False, 2);
  if APanel.Width > 0 then
    Result.FLabel^.show
  else
    Result.FLabel^.hide;
...
begin
  ...
  //GtkHBox inside GtkLayout for panel label packing.
  //NOT set as FCentralWidget so WidgetEvent is not connected to it,
  //all mouse handling is at the GtkLayout (FWidget) level only.
  FBox := TGtkHBox.new(GTK_ORIENTATION_HORIZONTAL, 1);
  PGtkBox(FBox)^.set_homogeneous(False);
  gtk_layout_put(PGtkLayout(Result), FBox, 0, 0);
  AContext := gtk_widget_get_style_context(FBox);
  ...
  begin
    ...
    ConnectPanelEvents(FSimplePanel, -1);
    gtk_box_pack_start(PGtkBox(FBox), FSimplePanel, True, True, 0);
    FSimplePanel^.show;
  end else
  begin
    for i := 0 to TStatusBar(LCLObject).Panels.Count - 1 do
      FPanels.AddPanel(TStatusBar(LCLObject).Panels[i], PGtkBox(FBox));
    FixLastPanelExpand;
  end;
end;
...
function TGtk3StatusBar.getClientOffset: TPoint;
begin
  Result := Point(0, 0);
end;
...
begin
  ...
  // Register bevel CSS once per process so label.statusbar-bevel-* classes work.
  if not Gtk3StatusBarBevelProviderInstalled then
  begin
    AProvider := gtk_css_provider_new;
    gtk_css_provider_load_from_data(AProvider, Gtk3StatusBarBevelCSS, -1, nil);
    gtk_style_context_add_provider_for_screen(gdk_screen_get_default,
      PGtkStyleProvider(AProvider), GTK_STYLE_PROVIDER_PRIORITY_APPLICATION);
    g_object_unref(AProvider);
    Gtk3StatusBarBevelProviderInstalled := True;
  end;
  // Keep FBox (GtkHBox inside GtkLayout) sized to fill the layout on resize.
  g_signal_connect_data(PGObject(FWidget), 'size-allocate',
    TGCallback(@StatusBarLayoutSizeAllocate), Self, nil, G_CONNECT_DEFAULT);
end;

class function TGtk3StatusBar.StatusBarPanelDraw(AWidget: PGtkWidget;
  ACairo: PCairo_t; AData: GPointer): GBoolean; cdecl;
var
...
begin
  ...
  AStatusBar := TGtk3StatusBar(AData);
  LCLBar := TStatusBar(AStatusBar.LCLObject);

  APanelIndex := Integer(PtrInt(g_object_get_data(PGObject(AWidget),
    'lcl_statusbar_panel_index')));
  if APanelIndex < 0 then
    exit;

  if LCLBar.SimplePanel then
    exit;

  if not ((APanelIndex >= 0) and (APanelIndex < LCLBar.Panels.Count)) then
    exit;

  if LCLBar.Panels[APanelIndex].Style <> psOwnerDraw then
    exit; //psText: let GtkLabel render the text
  ...
  Msg.Msg := LM_DRAWITEM;
  Msg.Ctl := LCLBar.Handle;
  Msg.DrawItemStruct := @AItemStruct;

  try
    AStatusBar.DeliverMessage(Msg, True);
    Result := gtk_true; //suppress default GtkLabel text rendering
  finally
    ADC.Free;
  end;
end;

class procedure TGtk3StatusBar.StatusBarLayoutSizeAllocate(AWidget: PGtkWidget;
  AGdkRect: PGdkRectangle; AData: gpointer); cdecl;
var
  AStatusBar: TGtk3StatusBar;
  Alloc: TGtkAllocation;
begin
  AStatusBar := TGtk3StatusBar(AData);
  if not Assigned(AStatusBar.FBox) then Exit;
  Alloc.x := 0;
  Alloc.y := 0;
  Alloc.width := Max(1, AGdkRect^.width);
  Alloc.height := Max(1, AGdkRect^.height);
  AStatusBar.FBox^.size_allocate(@Alloc);
end;

class function TGtk3StatusBar.SizeGripDraw(AWidget: PGtkWidget;
  ACairo: PCairo_t; AData: gpointer): GBoolean; cdecl;
var
  W, H, i: Integer;
  Offset: Double;
begin
  ...
  cairo_set_antialias(ACairo, CAIRO_ANTIALIAS_NONE);
  cairo_set_line_width(ACairo, 1.0);
  for i := 0 to 2 do
  begin
    Offset := (i + 1) * 4;
    //Shadow — dark line at position
    cairo_set_source_rgba(ACairo, 0.0, 0.0, 0.0, 0.5);
    cairo_move_to(ACairo, W - Offset, H - 2);
    cairo_line_to(ACairo, W - 2, H - Offset);
    cairo_stroke(ACairo);
    //Highlight — light line shifted 1px toward lower-right.
    cairo_set_source_rgba(ACairo, 1.0, 1.0, 1.0, 0.6);
    cairo_move_to(ACairo, W - Offset + 1, H - 2);
    cairo_line_to(ACairo, W - 2, H - Offset + 1);
    cairo_stroke(ACairo);
  end;
end;

class function TGtk3StatusBar.SizeGripButtonPress(AWidget: PGtkWidget;
...
begin
  ...
  if AEvent^.button <> 1 then
    exit;
  ATopLevel := gtk_widget_get_toplevel(AWidget);
  if not Assigned(ATopLevel) or not gtk_widget_is_toplevel(ATopLevel) then
    exit;
  PGdkWindow(gtk_widget_get_window(ATopLevel))^.begin_resize_drag(GDK_WINDOW_EDGE_SOUTH_EAST, AEvent^.button,
    Round(AEvent^.x_root), Round(AEvent^.y_root), AEvent^.time);
  Result := gtk_true;
end;

class procedure TGtk3StatusBar.SizeGripRealize(AWidget: PGtkWidget;
  AData: gpointer); cdecl;
var
  ACursor: PGdkCursor;
begin
  ACursor := gdk_cursor_new_for_display(gdk_display_get_default,
    GDK_BOTTOM_RIGHT_CORNER);
  gdk_window_set_cursor(gtk_widget_get_window(AWidget), ACursor);
  g_object_unref(ACursor);
end;

procedure TGtk3StatusBar.SetSizeGrip(AValue: Boolean);
begin
  if AValue then
  begin
    ...
    g_signal_connect_data(PGObject(FSizeGrip), 'button-press-event',
      TGCallback(@SizeGripButtonPress), Self, nil, G_CONNECT_DEFAULT);
    g_signal_connect_data(PGObject(FSizeGrip), 'realize',
      TGCallback(@SizeGripRealize), Self, nil, G_CONNECT_DEFAULT);
    gtk_box_pack_end(PGtkBox(FBox), FSizeGrip, False, False, 0);
    FSizeGrip^.show;
  end else
  begin
...
procedure TGtk3StatusBar.FixLastPanelExpand;
begin
  if FPanels.Count = 0 then
    exit;
  PGtkBox(FBox)^.set_child_packing(
    PGtkWidget(FPanels.Panels[FPanels.Count - 1].FLabel),
    True, True, 2, GTK_PACK_START);
end;

procedure TGtk3StatusBar.ConnectPanelEvents(ALabel: PGtkLabel; APanelIndex: Integer);
begin
  g_object_set_data(PGObject(ALabel), 'lcl_statusbar_panel_index',
    Pointer(PtrInt(APanelIndex)));

  g_signal_connect_data(PGtkWidget(ALabel), 'draw',
    TGCallback(@StatusBarPanelDraw), Self, nil, G_CONNECT_DEFAULT);
end;

procedure TGtk3StatusBar.PanelUpdate(AStatusBar: TStatusBar; APanelIndex: Integer);
var
  Panel: TGtk3StatusBarPanel;
  APanel: TStatusPanel;
begin
  if (APanelIndex < 0) or (APanelIndex >= FPanels.Count) then
    Exit;
  Panel := FPanels.Panels[APanelIndex];
  APanel := AStatusBar.Panels[APanelIndex];
  Panel.FLabel^.set_text(PgChar(APanel.Text));
  case APanel.Alignment of
    taCenter: Panel.FLabel^.set_xalign(0.5);
    taRightJustify: Panel.FLabel^.set_xalign(1.0);
  else
    Panel.FLabel^.set_xalign(0.0);
  end;
  if APanel.Width > 0 then
  begin
    Panel.FLabel^.set_size_request(APanel.Width, -1);
    Panel.FLabel^.show;
  end else
  begin
    Panel.FLabel^.set_size_request(0, -1);
    Panel.FLabel^.hide;
  end;
  with gtk_widget_get_style_context(PGtkWidget(Panel.FLabel))^ do
  begin
    remove_class('statusbar-bevel-lowered');
    remove_class('statusbar-bevel-raised');
    case APanel.Bevel of
      pbLowered: add_class('statusbar-bevel-lowered');
      pbRaised: add_class('statusbar-bevel-raised');
    end;
  end;
end;
...
begin
  ...
  begin
    for i := 0 to AStatusBar.Panels.Count - 1 do
      FPanels.AddPanel(AStatusBar.Panels[i], PGtkBox(FBox));
    FixLastPanelExpand;
  end;
end;
...
begin
  ...
  if not Assigned(FCentralWidget) then
  begin
    Result := inherited getClientRect;
    Exit;
  end;

  gtk_widget_get_allocation(FCentralWidget, @Alloc);

  (*This killed me for weeks:
   Translate (0,0) of FCentralWidget into the GtkNotebook coordinate system.
   (vX, vY) is where the content area starts relative to the notebook origin,
   i.e. the tab-bar width (LEFT tabs) or height (TOP tabs) including CSS borders.
   GTK3 over-allocates FCentralWidget to the full notebook rect when the tab bar
   precedes the content (TOP: vY > 0, LEFT: vX > 0). Subtracting the offsets
   gives the true client dimensions available to LCL children.
...
//Find the fsMDIForm whose TGtk3MDIWorkspace should host this fsMDIChild.
function FindMDIParentWorkspace(AChild: TCustomForm): TGtk3MDIWorkspace;
var
  i: Integer;
  F: TCustomForm;
  Win: TGtk3Window;
begin
  Result := nil;
  //TODO: Better check for child mdiarea host.
  ...
  if Screen <> nil then
    ...
    begin
      ...
      if (F.FormStyle = fsMDIForm) and F.HandleAllocated and (F <> AChild) then
      begin
...
function TGtk3Window.IsMdiForm: Boolean;
begin
  Result := (LCLObject is TCustomForm)
    and (TCustomForm(LCLObject).FormStyle = fsMDIForm)
    and not (csDesigning in LCLObject.ComponentState);
end;

function TGtk3Window.IsMdiChild: Boolean;
begin
  Result := (LCLObject is TCustomForm)
    and (TCustomForm(LCLObject).FormStyle = fsMDIChild)
    and not (csDesigning in LCLObject.ComponentState);
end;

function TGtk3Window.MDIWorkspace: TGtk3MDIWorkspace;
begin
  if FMDIArea <> nil then
    Result := FMDIArea.Workspace
  else
    Result := nil;
end;

procedure TGtk3Window.MDIChildActivated(AFrame: TGtk3MDIChildFrame);
var
  ChildWin: TGtk3Window;
  Msg: TLMessage;
begin
  if AFrame = nil then
    exit;
...
procedure TGtk3Window.MDIChildDeactivated(AFrame: TGtk3MDIChildFrame);
var
  ChildWin: TGtk3Window;
  Msg: TLMessage;
begin
...
function TGtk3Window.MDIChildCloseQuery(AFrame: TGtk3MDIChildFrame): Boolean;
var
  ChildWin: TGtk3Window;
  Msg: TLMessage;
begin
...
procedure TGtk3Window.MDIChildStateChanged(AFrame: TGtk3MDIChildFrame);
var
  ChildWin: TGtk3Window;
  Msg: TLMSize;
  Bnds: TRect;
begin
  ...
  case AFrame.State of
    ...
    mwsMaximized: Msg.SizeType := SIZE_MAXIMIZED;
  else
    Msg.SizeType := SIZE_RESTORED;
  end;
  Msg.SizeType := Msg.SizeType or Size_SourceIsInterface;
  Bnds := AFrame.GetBounds;
  Msg.Width := Word(Bnds.Right - Bnds.Left);
  Msg.Height := Word(Bnds.Bottom - Bnds.Top);
  ChildWin.DeliverMessage(Msg);
end;

procedure TGtk3Window.MDINoActiveChild(Sender: TObject);
var
  Msg: TLMessage;
begin
  if LCLObject = nil then
...
begin
  ...
  if (not Assigned(LCLObject.Parent)) and (not (wtMDIChild in FWidgetType)) then
  begin
...
begin
  ...
  if IsMdiForm and (FMDIArea = nil) then
  begin
    FMDIArea := TGtk3MDIArea.Create(LCLObject);
    FMDIArea.Workspace.OnChildActivated := @MDIChildActivated;
    FMDIArea.Workspace.OnChildDeactivated := @MDIChildDeactivated;
    FMDIArea.Workspace.OnChildCloseQuery := @MDIChildCloseQuery;
    FMDIArea.Workspace.OnChildStateChanged := @MDIChildStateChanged;
    FMDIArea.Workspace.OnNoActiveChild := @MDINoActiveChild;
    FMDIArea.Workspace.OwnerForm := LCLObject;
    if FCentralWidget <> nil then
    begin
      gtk_layout_put(PGtkLayout(FCentralWidget), FMDIArea.Widget, 0, 0);
      if (LCLObject.Width > 0) and (LCLObject.Height > 0) then
        PGtkWidget(FMDIArea.Widget)^.set_size_request(
          LCLObject.Width, LCLObject.Height);
      if FScrollWin <> nil then
        g_signal_connect_data(PGObject(FScrollWin), 'size-allocate',
          TGCallback(@MDILayoutSizeAllocate), Self, nil, G_CONNECT_DEFAULT);
    end;
    FWidgetType := FWidgetType + [wtMDIForm];
  end;
end;

class procedure TGtk3Window.MDILayoutSizeAllocate(AWidget: PGtkWidget;
  AAlloc: PGdkRectangle; AData: gpointer); cdecl;
var
  Win: TGtk3Window absolute AData;
  Parent: TWinControl;
  Ctrl: TControl;
  i, TopOff, BottomOff, LeftOff, RightOff: Integer;
  WSX, WSY, WSW, WSH: Integer;
  WS: TGtk3MDIWorkspace;
  Frame: TGtk3MDIChildFrame;
begin
  if (Win = nil) or (Win.FMDIArea = nil) or (Win.LCLObject = nil) then
    exit;

  {Reserve space for sibling LCL controls aligned to the form edges e.g.TStatusBar alBottom, toolbars alTop etc
   The MDI workspace must NOT cover them, minimised children dock to the workspace's bottom and wuld otherwise sit on top of the status bar.
   //TODO: Better idea to do this ?}

  Parent := TWinControl(Win.LCLObject);
  TopOff := 0;
  BottomOff := 0;
  LeftOff := 0;
  RightOff := 0;
  ...
  begin
    ...
    if not Ctrl.Visible then
      continue;
    case Ctrl.Align of
      alTop: Inc(TopOff, Ctrl.Height);
      alBottom: Inc(BottomOff, Ctrl.Height);
      alLeft: Inc(LeftOff, Ctrl.Width);
      alRight: Inc(RightOff, Ctrl.Width);
    end;
  end;

  WSX := LeftOff;
  WSY := TopOff;
  WSW := AAlloc^.width - LeftOff - RightOff;
  WSH := AAlloc^.height - TopOff - BottomOff;
  ...
  if (Win.FMDIArea.FAppliedX = WSX) and (Win.FMDIArea.FAppliedY = WSY)
    and (Win.FMDIArea.FAppliedW = WSW) and (Win.FMDIArea.FAppliedH = WSH) then
      exit;

  Win.FMDIArea.FAppliedX := WSX;
  Win.FMDIArea.FAppliedY := WSY;
  Win.FMDIArea.FAppliedW := WSW;
  Win.FMDIArea.FAppliedH := WSH;

  if Win.FCentralWidget <> nil then
    gtk_layout_move(PGtkLayout(Win.FCentralWidget), Win.FMDIArea.Widget, WSX, WSY);

  PGtkWidget(Win.FMDIArea.Widget)^.set_size_request(WSW, WSH);

  WS := Win.FMDIArea.Workspace;
  if WS <> nil then
  begin
    for i := 0 to WS.Children.Count - 1 do
    begin
      Frame := TGtk3MDIChildFrame(WS.Children[i]);
      if Frame.State = mwsMaximized then
        Frame.RefitMaximized(WSW, WSH);
    end;
...
function TGtk3MDIChildWindow.CreateWidget(const Params: TCreateParams): PGtkWidget;
var
  AForm: TCustomForm;
  Workspace: TGtk3MDIWorkspace;
  InnerForm: PGtkWidget;
begin
  AForm := TCustomForm(LCLObject);
  Workspace := FindMDIParentWorkspace(AForm);
  if Workspace = nil then
  begin
    //No MDI parent yet, so fallback to normal toplevel.
    Result := inherited CreateWidget(Params);
    exit;
  end;
  FWidgetType := FWidgetType + [wtMDIChild];
  InnerForm := inherited CreateWidget(Params);

  if InnerForm <> nil then
    PGtkWidget(InnerForm)^.set_can_focus(False);

  if FScrollWin <> nil then
    PGtkWidget(FScrollWin)^.set_can_focus(False);

  FMDIFrame := Workspace.AddChild(AForm.Caption);
  FMDIFrame.UserData := Self;
  FMDIFrame.SetBorderStyle(AForm.BorderStyle);

  //here we are
  FMDIFrame.AddClientHost(InnerForm);

  AForm.Left := FMDIFrame.GetBounds.Left;
  AForm.Top := FMDIFrame.GetBounds.Top;

  ApplyMDIPosition(AForm);

  Result := FMDIFrame.Frame;
  FWidget := Result;
end;

procedure TGtk3MDIChildWindow.InitializeWidget;
begin
  inherited InitializeWidget;
end;

procedure TGtk3MDIChildWindow.ApplyMDIPosition(AForm: TCustomForm);
var
  WS: TGtkAllocation;
  L, T: Integer;
begin

  if FMDIFrame = nil then
    exit;

  case AForm.Position of
    poDesigned, poDefaultPosOnly:
      FMDIFrame.SetBounds(AForm.Left, AForm.Top, AForm.Width, AForm.Height);
    poScreenCenter, poWorkAreaCenter, poDesktopCenter,
    poMainFormCenter, poOwnerFormCenter:
    begin
      if (FMDIFrame.Workspace = nil) or (FMDIFrame.Workspace.Widget = nil) then
        exit;
      PGtkWidget(FMDIFrame.Workspace.Widget)^.get_allocation(@WS);
      L := (WS.width  - AForm.Width)  div 2;
      T := (WS.height - AForm.Height) div 2;
      ...
      FMDIFrame.SetBounds(L, T, AForm.Width, AForm.Height);
...
procedure TGtk3MDIChildWindow.setText(const AValue: String);
begin
  FText := AValue;
  if FMDIFrame <> nil then
    FMDIFrame.Caption := AValue;
end;

destructor TGtk3MDIChildWindow.Destroy;
var
  WS: TGtk3MDIWorkspace;
begin
  if FMDIFrame <> nil then
  begin
    WS := FMDIFrame.Workspace;
    if WS <> nil then
      WS.RemoveChild(FMDIFrame);
    FMDIFrame := nil;
  end;
  inherited Destroy;
end;

procedure TGtk3MDIChildWindow.SetBounds(ALeft, ATop, AWidth, AHeight: Integer);
begin
  if FMDIFrame <> nil then
    FMDIFrame.SetBounds(ALeft, ATop, AWidth, AHeight)
  else
    inherited SetBounds(ALeft, ATop, AWidth, AHeight);
end;

procedure TGtk3MDIChildWindow.Activate;
begin
  if FMDIFrame <> nil then
    FMDIFrame.BringToFront
  else
    inherited Activate;
end;
...
constructor TGtk3MDIArea.Create(const AMDIForm: TWinControl);
begin
  FWorkspace := TGtk3MDIWorkspace.Create;
  inherited CreateFrom(AMDIForm, FWorkspace.Widget);
  FCentralWidget := FWorkspace.Container;
  FHasPaint := True;
...
destructor TGtk3MDIArea.Destroy;
begin
  ...
  //already destroyed in FWorkspace destructor, just nil to not propagate dangling pointer forward.
  FWidget := nil;
  FCentralWidget := nil;
  FOwnWidget := False;
  inherited Destroy;
...
function TGtk3MDIArea.getClientRect: TRect;
var
  Alloc: TGtkAllocation;
begin
  if FCentralWidget <> nil then
  begin
    PGtkWidget(FCentralWidget)^.get_allocation(@Alloc);
    Result := Rect(0, 0, Alloc.width, Alloc.height);
  end else
  if FWidget <> nil then
  begin
