...
begin
  ...
  writeln('');
  writeln('--mistrust');
  w(lisAbortIfAnyCompilerPathOrExecut);
  writeln('');
  writeln('--build-ide, --build-ide=<options>');
  w(lisBuildIDEWithPackages);
  writeln('');
  writeln('--build-ide-minimal');
  w(lisBuildTheMinimalIDEWithDefaults);
  writeln('');
  writeln('--build-ide-release');
  w(lisBuildTheReleaseIDESameAsMinima);
  writeln('');
  writeln('--pkg-release');
  w(lisBuildReleasePackagesStoreCompi);
  writeln('');
  writeln('-v, --version');
  ...
  w(Format(lisOverrideTheProjectWidgetsetEGDefault, ['gtk3 qt qt5 qt6 win32 cocoa', LCLPlatformDirNames[GetBuildLCLWidgetType]]));
  writeln('');
  writeln('--cpu=<cpu>');
